...
import { ArrowUpCircle, Loader2, Trash2, RefreshCw } from 'lucide-react'
import { cn } from '@/lib/utils'
import type { KasKeluarRow } from '@/types/database.types'

const KATEGORI_LABEL: Record<string, { label: string; color: string }> = {
  sewa:          { label: 'Sewa',                  color: 'bg-orange-100 text-orange-700 border-orange-200' },
  listrik_air:   { label: 'Listrik & Air',          color: 'bg-yellow-100 text-yellow-700 border-yellow-200' },
  gaji:          { label: 'Gaji / Upah',            color: 'bg-teal-100 text-teal-700 border-teal-200' },
  alat:          { label: 'Alat & Peralatan',       color: 'bg-blue-100 text-blue-700 border-blue-200' },
  barang_habis:  { label: 'Barang Habis Pakai',     color: 'bg-purple-100 text-purple-700 border-purple-200' },
  transport_ops: { label: 'Transport Operasional',  color: 'bg-cyan-100 text-cyan-700 border-cyan-200' },
...
export default function KasKeluarPage() {
  ...
  const [rows, setRows]                 = useState<KasKeluarRow[]>([])
  const [isLoading, setIsLoading]       = useState(true)
  const [isSaving, setIsSaving]         = useState(false)
  const [deleteTarget, setDeleteTarget] = useState<KasKeluarRow | null>(null)
  ...
  const fetchData = useCallback(async () => {
    ...
    else setRows((data ?? []) as KasKeluarRow[])
    setIsLoading(false)
  }, [])

  useEffect(() => { fetchData() }, [fetchData])

  // State lokal untuk field Rupiah
  ...
  async function onSubmit(values: KasFormValues) {
    ...
    const { error } = await supabase.from('kas_keluar').insert({
      ...
      kategori:  values.kategori as KasKeluarRow['kategori'],
  ...
  async function handleDelete() {
    ...
    setIsDeleting(true)
    const { error } = await supabase.from('kas_keluar').delete().eq('id', deleteTarget.id)
    if (error) {
  ...
  const totalByKategori = (Object.keys(KATEGORI_LABEL) as KasKeluarRow['kategori'][]).reduce(
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          Catat pengeluaran operasional: sewa, listrik, gaji, alat, dan barang habis pakai
        </p>
      </div>
        ...
        <div className="lg:col-span-2">
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base">Tambah Kas Keluar</CardTitle>
              <CardDescription className="text-xs">
                Input pengeluaran operasional di luar biaya pembelian buah
              </CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      placeholder="Contoh: Sewa lapak pasar bulan April..."
              ...
              {grandTotal > 0 ? (
                <>
                  <div className="rounded-lg bg-destructive/5 border border-destructive/20 p-3 text-center">
                    <p className="text-xs text-muted-foreground mb-1">Total Kas Keluar</p>
                    <p className="text-xl font-bold text-destructive">{formatRupiahFull(grandTotal)}</p>
                  </div>
                  <Separator />
                  <div className="space-y-2">
                    {(Object.entries(KATEGORI_LABEL) as [KasKeluarRow['kategori'], typeof KATEGORI_LABEL[string]][]).map(([k, v]) =>
                      totalByKategori[k] > 0 ? (
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-semibold">Riwayat Kas Keluar</h2>
          ...
          <CardContent className="p-0">
            {isLoading ? (
              ...
              <div className="py-12 text-center">
                <ArrowUpCircle className="h-8 w-8 mx-auto text-muted-foreground opacity-30 mb-3" />
                <p className="text-sm text-muted-foreground">Belum ada data kas keluar</p>
              </div>
            ) : (
              <Table>
                <TableHeader>
          ...
          {deleteTarget && (
            <div className="rounded-lg border bg-muted/40 p-3 text-sm space-y-1.5">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-destructive">{formatRupiahFull(deleteTarget.jumlah)}</span>
              </div>
            </div>
          )}
          <DialogFooter>
