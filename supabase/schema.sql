...
-- Kas Masuk (Modal & Pemasukan Non-Operasional)
-- Mencatat injeksi modal, pinjaman, dan pemasukan non-penjualan-buah lainnya
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'allow_all_kas_masuk' AND tablename = 'kas_masuk') THEN
    CREATE POLICY "allow_all_kas_masuk" ON kas_masuk FOR ALL USING (true) WITH CHECK (true);
  END IF;
END $$;
...
-- Kas Keluar (Pengeluaran Operasional Non-Pembelian-Buah)
-- Mencatat sewa, listrik, gaji, alat, barang habis pakai, dan pengeluaran ops lainnya
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'allow_all_kas_keluar' AND tablename = 'kas_keluar') THEN
    CREATE POLICY "allow_all_kas_keluar" ON kas_keluar FOR ALL USING (true) WITH CHECK (true);
  END IF;
END $$;
